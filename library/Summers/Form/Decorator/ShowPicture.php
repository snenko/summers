 ...
 */
class Summers_Form_Decorator_ShowPicture extends Zend_Form_Decorator_Abstract
{

    /**
     * Получение строк подключения Javacript и CSS
...
//';
    ...
    public function render($content)
    {
        ...
        switch ($placement) {
            // После элемента
            case  'APPEND':
                return $this->_getJsAndCss() . $content . $separator . $this->_getImgBlock();
            // Перед элементом
            case  'PREPEND':
                return $this->_getJsAndCss() . $this->_getImgBlock() . $separator . $content;
            case  null:
                // По умолчанию просто возвращаем содержимое календаря
            default:
                return $this->_getJsAndCss() . $this->_getImgBlock();
        }
    }
}
